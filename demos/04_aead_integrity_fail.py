#!/usr/bin/env python3
# SPDX-License-Identifier: Apache-2.0
"""
PAXECT AEAD Hybrid Plugin — Demo 04: AEAD Integrity Fail Test
v1.0.2
Demonstrates that any corruption in ciphertext triggers a safe decryption failure.
"""

import os
import sys
import tempfile
import hashlib
import subprocess
from datetime import datetime, timezone

PLUGIN = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "paxect_aead_enterprise.py"))
PASSWORD = "demo123"


def sha256(path: str) -> str:
    with open(path, "rb") as f:
        return hashlib.sha256(f.read()).hexdigest()


def main():
    print("=== PAXECT AEAD Demo 04 — Integrity Fail Test ===")
    print(f"Local time : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"UTC time   : {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S %Z')}")

    tmp = tempfile.mkdtemp(prefix="paxect_demo04_")
    src = os.path.join(tmp, "input.txt")
    enc = os.path.join(tmp, "output.aead")
    cor = os.path.join(tmp, "corrupted.aead")
    dec = os.path.join(tmp, "decrypted.txt")

    # 1️⃣ Create input file
    with open(src, "w", encoding="utf-8") as f:
        f.write("PAXECT AEAD Demo 04 - Integrity verification test\n")
        f.write(f"Generated at UTC: {datetime.now(timezone.utc).isoformat()}\n")

    print(f"[1] Source file : {src}")

    # 2️⃣ Encrypt
    print(f"[2] Encrypting → {enc}")
    with open(src, "rb") as fin, open(enc, "wb") as fout:
        subprocess.run(
            [sys.executable, PLUGIN, "--mode", "encrypt", "--cipher", "auto", "--pass", PASSWORD],
            stdin=fin, stdout=fout, check=True
        )

    # 3️⃣ Corrupt ciphertext
    print(f"[3] Corrupting ciphertext → {cor}")
    data = bytearray(open(enc, "rb").read())
    if len(data) > 100:
        data[100] ^= 0xFF  # flip one byte
    else:
        data[-1] ^= 0x01
    with open(cor, "wb") as f:
        f.write(data)

    # 4️⃣ Attempt decryption (expected to fail)
    print("[4] Attempting decryption of corrupted file → expected to fail")
    try:
        with open(cor, "rb") as fin, open(dec, "wb") as fout:
            subprocess.run(
                [sys.executable, PLUGIN, "--mode", "decrypt", "--pass", PASSWORD],
                stdin=fin, stdout=fout, check=True, stderr=subprocess.PIPE
            )
        print("❌  ERROR: decryption succeeded unexpectedly!")
    except subprocess.CalledProcessError as e:
        print("✅  Integrity check triggered — decryption failed safely.")
        if e.stderr:
            err = e.stderr.decode(errors="ignore").strip()
            if err:
                print(f"AEAD reported error:\n{err}")
            else:
                print("(no stderr output)")

    print(f"\nTemporary files stored in: {tmp}")
    print("=== Demo 04 completed successfully ===")


if __name__ == "__main__":
    main()

